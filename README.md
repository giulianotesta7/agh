...
| Cache | `.agh/packs/`; downloaded pack files that `agh pull` can rebuild. |
...
Skill targets under `.claude/skills/` and `.opencode/skills/` are generated by `agh pull`. Commit them only if your team wants agent skills reviewed in Git. If they are symlinks, run `agh pull` after clone to refresh `.agh/packs/`.
