"""Project pack assignment route tests."""
...
def _pack_files(domain: str, name: str, version: str) -> dict[str, str]:
    return {
        "agh.pack.toml": (
            f'domain = "{domain}"\n'
            f'name = "{name}"\n'
...
def _publish_pack(client: TestClient, token: str, ref: str) -> dict[str, Any]:
    pair, version = ref.split("@", 1)
    domain, name = pair.split("/", 1)
    response = client.post(
        "/api/v1/packs",
        json={"files": _pack_files(domain, name, version)},
        headers=_auth(token),
    )
    assert response.status_code == 201, response.text
    return response.json()
...
def test_admin_assigns_concrete_and_latest_pack_versions_with_ordering(
    tmp_path: Path, monkeypatch
) -> None:
    client, owner_token = _client_with_owner(tmp_path, monkeypatch)
    project = _create_project(client, owner_token)
    _publish_pack(client, owner_token, "acme/onboarding@1.0.0")
    _publish_pack(client, owner_token, "acme/onboarding@1.2.0")
    _publish_pack(client, owner_token, "acme/baseline@1.0.0")

    latest = _assign_pack(
        client, owner_token, project["id"], "acme/onboarding@latest", position=20
    )
    concrete = _assign_pack(
    ...
    listed = client.get(
        f"/api/v1/projects/{project['id']}/packs", headers=_auth(owner_token)
    )
    assert listed.status_code == 200
    assert [item["pack_ref"] for item in listed.json()["project_packs"]] == [
        "acme/baseline@1.0.0",
...
def test_project_developer_lists_only_active_assignments(
    ...
    assert [item["id"] for item in visible.json()["project_packs"]] == [
        assignment.json()["id"]
    ]

    removed = client.delete(
        f"/api/v1/projects/{project['id']}/packs/{assignment.json()['id']}",
        headers=_auth(owner_token),
    )
    ...
    assert hidden.json() == {"project_packs": []}
...
def test_assignment_update_position_version_and_deactivation(
    ...
    patched = client.patch(
        f"/api/v1/projects/{project['id']}/packs/{assignment.json()['id']}",
        json={"pack_ref": "acme/onboarding@latest", "position": 7},
        headers=_auth(owner_token),
    )
