...
`AGENTS.md` and `CLAUDE.md` are optional by agent, but the current publisher requires at least one instruction file. Skills use one directory per skill name.
