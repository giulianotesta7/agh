...
- PR10B adds a public installer path: `scripts/install.sh` works when piped through `curl | sh` by installing the `agh` package with `uv tool install --force agh`, while preserving checkout installs from the repo root for development. README, installation docs, quickstarts, release checklist, and installer tests cover both paths.
