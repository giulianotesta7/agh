...
| File | Action | Notes |
|------|--------|-------|
| `agh/cli/main.py` | Modified | Adds top-level `agh pull` with `--dry-run` and `--force`, stable exit-code mapping, and JSON pull summaries. |
| `agh/cli/workspace_pull.py` | Modified | Adds project-link reading, pull-manifest fetching, no-write artifact download/validation, pull orchestration, atomic target writes, and cache/lock writing after successful apply. |
| `agh/cli/pull_markers.py` | Modified | Adds force planning that overwrites checksum-conflicted matching AGH-managed blocks only while preserving unmanaged content. |
| `agh/cli/pull_plan.py` | Modified | Threads `force` through dry-run-safe planning without changing default conflict behavior. |
| `tests/test_cli_pull.py` | Created | CLI coverage for strict no-write dry-run, normal target/cache/lock writes, conflict exit 3 no writes, force overwrite, missing link exit 5, auth exit 4, and invalid manifest/path exit 2. |
...
| File | Action | Notes |
|------|--------|-------|
| `agh/cli/workspace_pull.py` | Modified | Splits instruction and skill artifacts during pull; keeps instructions on managed markers, places skills as relative symlinks with copy fallback, preserves strict no-write dry-run, and records skill lock modes. |
| `tests/test_cli_pull.py` | Modified | Adds skill pull coverage for symlink placement, copy fallback, no managed markers in skill files, conflict no-write, force replacement, dry-run no-write, and lock `mode`. |
...
| `agh/cli/main.py` | Modified | Prints VCS guidance hint after successful non-dry-run pull when `.agh-cache/packs/` is not ignored. |
| `agh/cli/workspace_pull.py` | Modified | Adds git worktree and `.agh-cache/` ignore detection for pull result hints. |
...
uv run pytest tests/test_cli_pull.py -q
# 7 passed in 3.22s

uv run pytest tests/test_pull_markers.py tests/test_pull_plan.py tests/test_workspace_pull.py tests/test_cli_pull.py -q
# 66 passed in 5.26s

uv run --with pyright pyright agh/cli/main.py agh/cli/pull_markers.py agh/cli/pull_plan.py agh/cli/workspace_pull.py tests/test_cli_pull.py
# 0 errors, 0 warnings, 0 informations

uv run pytest -q
# 163 passed, 1 warning in 19.36s
...
uv run pytest tests/test_workspace_pull.py tests/test_cli_pull.py -q
# 45 passed in 5.23s (after PR5D first review fixes)

uv run pytest -q
# 169 passed, 1 warning in 19.57s (after PR5D first review fixes)

uv run --with pyright pyright agh/cli/main.py agh/cli/pull_markers.py agh/cli/pull_plan.py agh/cli/workspace_pull.py tests/test_cli_pull.py tests/test_workspace_pull.py
# 0 errors, 0 warnings, 0 informations
...
uv run pytest tests/test_workspace_pull.py tests/test_cli_pull.py -q
# 47 passed in 5.24s (after PR5D second review fix)

uv run pytest -q
# 171 passed, 1 warning in 19.63s (after PR5D second review fix)
...
uv run pytest tests/test_workspace_pull.py tests/test_cli_pull.py -q
# 49 passed in 5.24s (after PR5D third review fix)

uv run pytest -q
# 173 passed, 1 warning in 19.47s (after PR5D third review fix)
...
uv run pytest -q
# 183 passed, 1 warning in 22.45s

uv run --with pyright pyright agh/cli/agent_integrations.py agh/cli/main.py agh/cli/workspace_pull.py tests/test_cli_pull.py tests/test_workspace_pull.py
# 0 errors, 0 warnings, 0 informations
...
uv run pytest tests/test_cli_pull.py -q
# 16 passed in 7.84s (after PR6B first review fix)

uv run pytest tests/test_workspace_pull.py tests/test_cli_pull.py -q
# 58 passed in 9.90s (after PR6B first review fix)

uv run pytest -q
# 186 passed, 1 warning in 24.08s (after PR6B first review fix)
...
# 8 passed in 0.01s (after PR7A CLI installer)

uv run pytest -q
# 198 passed, 1 warning in 26.53s (after PR7A CLI installer)
...
- PR5C fresh/security review found lockfiles used absolute cache paths, cache writes could follow symlinked `.agh-cache/packs` or intermediate directories, malformed non-object manifest entries were silently ignored, and control characters could make invalid TOML. Fixed lock `source` to repo-relative `.agh-cache/packs/...`, added symlink ancestor boundary checks before download/write, rejected non-object packs/artifacts, and rejected control characters in manifest fields/TOML values; added regressions.
- PR5C second review found control-character validation still missed non-NUL controls, empty paths passed as `.`, and `_lock_source_path()` could choose an ancestor `.agh` instead of workspace `.agh`. Fixed control validation to reject all C0/DEL controls, reject empty paths, prevalidate lock metadata before downloads, and store `CachedArtifact.cache_path` relative to the workspace for unambiguous `.agh-cache/packs/...` lock sources; added regressions.
...
- PR5D implementation split pull downloads into a no-write `download_manifest_artifacts()` path so `agh pull --dry-run` can fetch/verify artifact content for planning without creating `.agh-cache/packs`, `.agh/lock.toml`, or target files. `populate_cache_and_write_lock()` now preflights cache symlink boundaries before network downloads to preserve PR5C safety behavior.
- PR6B routes `kind="skill"` artifacts away from managed marker planning. Skill files are placed as real files via relative symlink to `.agh-cache/packs/...` with copy fallback, existing unmanaged targets conflict with exit 3 unless `--force`, and lock entries record `mode="symlink"` or `mode="copy"` while preserving cache `source` provenance.
- PR6C first review found `git check-ignore` checked `.agh-cache` without a trailing slash, which can miss a `.gitignore` rule `.agh-cache/packs/` before the directory exists for empty manifests. Fixed the ignore check to query `.agh-cache/` and added an empty-manifest regression.
...
- PR6D adds bounded integration/API contract coverage for task 6.4: a live-server smoke test covers owner auth, project create, pack publish, assignment, pull manifest, `agh sync`, `agh pull --dry-run`, `agh pull`, VCS hint output, `.agh/project.toml`, `.agh/lock.toml`, `.agh-cache/packs/`, instruction targets, and skill placement. API error tests document the current FastAPI-style JSON `detail` behavior for unauthenticated, forbidden, missing, validation, duplicate, and conflict cases.
- Future API polish follow-up: decide whether to keep FastAPI's mixed `detail` strings/lists or introduce an AGH-specific error envelope. Current behavior is now tested rather than redesigned in PR6D.
- PR8A starts CLI success-output polish with `agh sync`: successful sync now prints concise human-readable lines instead of a JSON object, while preserving errors and exit behavior. Remaining future CLI polish: `agh pull`, read/list commands, and mutating admin commands should also move away from API-shaped success output; add machine-readable output later as an explicit option if needed.
- PR8B continues CLI success-output polish with `agh pull`: successful pulls, dry-runs, and conflicts now print concise human-readable summaries with path lists instead of JSON-shaped output, while preserving exit codes, dry-run no-write behavior, force behavior, cache/lock internals, and the optional `.agh-cache/` VCS hint.
- PR8C moves generated pull cache out of committed `.agh/` metadata before 0.1.0. New pulls write `.agh-cache/packs/...`, lock `source` paths use `.agh-cache/packs/...`, VCS hints recommend ignoring `.agh-cache/`, and docs explain old pre-release `.agh/packs/` caches can be deleted after a current pull. No automatic migration is needed before 0.1.0.
- PR8D continues CLI success-output polish for list commands: `agh project list`, `agh pack list`, `agh project pack list`, and `agh user list` now print concise human-readable tables with typed headers. `agh project get` and mutating/token-sensitive commands intentionally keep their existing JSON/redaction behavior for later slices.
- PR8E continues CLI success-output polish for project mutation commands: `agh project create/update/delete`, `agh project member add/remove`, and `agh project pack add/update/remove` now print concise human-readable success messages. User/token and `agh pack publish` commands intentionally keep existing JSON/redaction behavior for later slices.
- PR8F continues CLI success-output polish for `agh project get` and `agh pack publish`: project detail and successful pack publish now print concise human-readable summaries while preserving existing errors and avoiding secret/hash fields in publish output.
- PR8G completes CLI success-output polish for token-sensitive user/token mutations: `agh user create/update/delete` and `agh token rotate/reset` now print human-readable summaries, plaintext one-time tokens are still shown exactly where required, and hash/stored config token fields remain hidden.
- PR9A adds core-flow English docs for packs, projects, and admin workflows. README stays a landing page and links to `docs/packs.md`, `docs/projects.md`, and `docs/admin.md`; docs tests cover key commands, manifest requirements, assignment IDs, `latest` resolution, roles, and one-time token behavior.
- PR9B adds Spanish docs: `README.es.md` and `docs/es/*` mirror the current English guides while preserving commands, paths, CLI output, env vars, URLs, `.agh-cache/`, `scripts/install.sh`, and current pack manifest behavior.
- PR10A release readiness audit found core technical checks passing (`pytest`, `ruff`, lockfile, CLI entrypoint, Dockerfile syntax) and added `docs/release-checklist.md` for pre-tag validation. The audit identified pre-0.1.0 work around public installer without clone, skill-only packs, `agh pack init`, release docs updates, and explicit decisions on agent selection and API error envelope.
- PR10B adds a public installer path: `scripts/install.sh` works when piped through `curl | sh` by installing the `agh` package with `uv tool install --force agh`. Development checkout installs are documented separately as `uv tool install --force .`. README, installation docs, quickstarts, release checklist, and installer tests cover both paths.
- PR10C adds CI-only GitHub Actions validation on pull requests and pushes to `main`: lockfile check, tests, Ruff lint, Ruff format check, Pyright, Dockerfile check, package build, built wheel install, and `agh --help`. It intentionally does not publish packages or Docker images.
- PR10D allows skill-only packs. Pack publish validation now requires at least one instruction file or skill, not specifically `AGENTS.md`/`CLAUDE.md`; client, server, pull-manifest, docs, and tests cover the skill-only path while empty packs remain invalid.
- PR10E adds `agh pack init` for local pack template scaffolding. By default it creates only `agh.pack.toml`, `instructions/`, and `skills/`; flags can customize description and optionally create `AGENTS.md`, `CLAUDE.md`, or one or more `skills/<name>/SKILL.md` starter files. `--version` is required so immutable pack versions stay explicit.
- PR10F adds a manual PyPI publish workflow using Trusted Publishing (`id-token: write`, environment `pypi`, and `pypa/gh-action-pypi-publish`). It validates package name, lockfile, tests, Ruff, Pyright, builds the package, installs the built wheel, and checks `agh --help` before publishing. Agent selection wizard and AGH-specific error envelope are explicitly deferred after `0.1.0`.
- PR10G adds a manual `cd-ghcr.yml` container publish workflow. It requires `confirm=publish`, validates that the requested version matches `pyproject.toml`, checks the Dockerfile, logs into GHCR with `GITHUB_TOKEN`, and publishes `ghcr.io/giulianotesta7/agent-guidance-hub:<version>` plus `latest`. It does not run on push.
- Post-release docs update adds `docker-compose.yml` with the published GHCR image and updates README/install/quickstart/operations docs in English and Spanish so production starts with `docker compose up -d`; local `docker build` remains a development path.
- Release automation WIP switches package versioning to `setuptools-scm`, adds a tag-driven `.github/workflows/release.yml` that runs validation, publishes PyPI and GHCR, and creates GitHub Releases from `v*` tags. The previous manual PyPI/GHCR workflows are removed so the normal release path is one tag push. Docker builds receive `AGH_VERSION` as a build arg because `.git` is excluded from Docker context. Review-found recovery and `latest` risks are handled by splitting PyPI/GHCR/GitHub Release into separate jobs and requiring the release tag to point at `origin/main` and be the highest SemVer tag before publishing; GHCR re-checks the same guard immediately before pushing `latest` so rerun-failed-jobs cannot promote an older tag after a newer release exists. Compose image selection now uses `${AGH_IMAGE_TAG:-latest}` so docs do not require per-release commits while production can still pin `AGH_IMAGE_TAG=0.1.2`.
...
- PR5D third security review found `manifest.project.id` still used `str(...)` coercion for lock output, allowing non-string project IDs into `.agh/lock.toml`. Fixed project metadata validation to reject non-object projects and present non-string IDs, and lock rendering to require string IDs; added regressions.
