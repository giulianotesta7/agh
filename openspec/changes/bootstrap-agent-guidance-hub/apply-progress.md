...
- Post-release docs update adds `compose.yaml` with the published GHCR image and updates README/install/quickstart/operations docs in English and Spanish so production starts with `docker compose up -d`; local `docker build` remains a development path.
