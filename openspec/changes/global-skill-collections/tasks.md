...
- [x] 3.2c Harden global skill remove and lock parsing against tampered lock paths, unsafe lock-update ordering, and malformed lock state. _(PR 2A.1c: remove/lock resilience)_
- [x] 3.3 Add CLI commands for `skill list/install/remove` plus global-skill-scoped agent defaults such as `skill agent show/select/clear` in `agh/cli/main.py`. _(PR 2A.2: CLI commands)_
...
- [x] 4.2c Add behavior-first hardening tests for tampered remove `target_path`, lock-update failure recovery, and malformed lock/list behavior. _(PR 2A.1c: remove/lock resilience)_
- [x] 4.3 Verify CLI prompts and default-agent behavior, including `Select the agent for global skills:` wording. _(PR 2A.2: CLI tests)_

## Phase 5: Cleanup / Documentation

- [x] 5.1 Update CLI help/docs for global skill commands and agent-default behavior.
- [x] 5.2 Keep the workspace prompt wording cleanup as a separate follow-up PR; do not include it in core implementation. _(Deferred to follow-up issue #109; not part of core implementation.)_
